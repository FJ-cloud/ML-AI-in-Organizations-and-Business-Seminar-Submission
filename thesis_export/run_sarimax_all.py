...
def main():
    ...
    p.add_argument('--top_k', type=int, default=0, help='If >0, compute and report top-K features by abs(coef)')
    args = p.parse_args()
    ...
    if args.top_k and args.top_k > 0:
        ...
        top_features = imp[imp['feature'] != 'sarimax_score']['feature'].head(args.top_k).tolist()
    ...
    out_path = OUT_DIR / f'hybrid_sarimax_l1_metrics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
