...
class Config:
    ...
    horizon_months: int = 0  # 0 = contemporaneous labeling as in current dataset
    roll_windows: tuple[int, ...] = (3, 6)
    feature_freq_threshold: float = 0.6  # keep features selected in >=60% folds
    use_catboost: bool = True
    target_col: str = 'failed'  # can be 'failed' or early-warning like 'will_fail_within_1m'
    drop_post_failure: bool = False
...
def latest_model_lag1() -> Path:
    ...
    if not candidates:
        raise SystemExit('No model_lag1_*.csv found. Expected under data/processed/model_ready relative to this script or CWD.')
...
def canonicalize_macros(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep non-z canonical (level) and drop the z-duplicate for modeling; we can restandardize later
...
def get_features(df: pd.DataFrame) -> tuple[list[str], str]:
    # y_col now set by config
...
def stability_selection(train_df: pd.DataFrame, feat_cols: list[str], y_col: str, n_splits: int = 5, threshold: float = 0.6) -> tuple[list[str], dict[str, int]]:
    ...
    for i in range(n_splits):
        ...
        if y.nunique() < 2:
            # skip slices with single-class targets
            continue
        valid_slices += 1
        # Simple L1 logistic for selection
        X = tr[feat_cols].astype(float).values
        y = y.values
        # Impute + scale for stability of L1
...
def run(cfg: Config, csv: str | None, top_k: int = 0, use_all: bool = False):
    ...
    # Optionally drop post-failure months when doing early-warning targets
    ...
    if use_all:
        ...
        # Rank by stability frequency desc, then name
    ...
    # Optionally save per-row predictions
    if cfg.save_preds:
...
def main():
    ...
    ap.add_argument('--top_k', type=int, default=0, help='If >0, limit to top-K features by stability frequency')
    ap.add_argument('--use_all', action='store_true', help='Use all engineered features (skip stability selection)')
    ap.add_argument('--target', type=str, default='failed', help='Target column, e.g., failed or will_fail_within_1m')
    ap.add_argument('--drop_post_failure', action='store_true', help='Drop months on/after failure (useful for early-warning)')
    ap.add_argument('--save_preds', action='store_true', help='Save per-row predictions')
    args = ap.parse_args()
    cfg = Config(split_date=args.split_date, use_catboost=not args.no_cat, target_col=args.target, drop_post_failure=args.drop_post_failure, save_preds=args.save_preds)
    run(cfg, args.csv, top_k=args.top_k, use_all=args.use_all)
